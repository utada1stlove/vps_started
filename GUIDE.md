# VPS Setup Guide / VPS 配置指南

This guide walks you through setting up a fresh VPS from scratch, in the recommended order.

本指南按推荐顺序，带你一步步配置一台全新的 VPS。

---

## Step 1 — Reinstall the OS (DD) / 重装系统

**Why?**
Most VPS providers ship outdated or bloated OS images. Reinstalling gives you a clean, minimal Debian 13.

**为什么要做？**
很多 VPS 商家预装的系统版本旧或带有多余软件。重装可以得到一个干净的 Debian 13 环境。

> You will lose SSH access during reinstall. The VPS will reboot into the new system automatically. Reconnect with the password you set below.
>
> 重装过程中 SSH 会断开，VPS 会自动重启进入新系统。用你下面设置的密码重新连接。

**If your VPS is in China / 国内服务器：**

```bash
curl -O https://cnb.cool/bin456789/reinstall/-/git/raw/main/reinstall.sh || wget -O ${_##*/} $_
bash reinstall.sh debian 13 --ssh-port 22 --password 'YourPasswordHere'
```

**If your VPS is outside China / 国外服务器：**

```bash
curl -O https://raw.githubusercontent.com/bin456789/reinstall/main/reinstall.sh || wget -O ${_##*/} $_
bash reinstall.sh debian 13 --ssh-port 22 --password 'YourPasswordHere'
```

> Replace `YourPasswordHere` with a strong password. You will use it to SSH in after reboot.
>
> 将 `YourPasswordHere` 替换为强密码，重启后用它登录 SSH。

After reboot, reconnect: / 重启后重新连接：

```bash
ssh root@<your-vps-ip> -p 22
```

---

## Step 2 — SSH Public Key / 配置 SSH 公钥登录

**Why?**
Key-based auth is more secure than passwords and lets you log in without typing a password.

**为什么要做？**
公钥登录比密码更安全，配置后无需每次输入密码。

### 2a — Add public key to the server / 写入公钥

Run this on the VPS: / 在 VPS 上执行：

```bash
curl -fsSL https://raw.githubusercontent.com/utada1stlove/vps_started/main/setup_pubkey.sh | bash
```

Or manually: / 或手动执行：

```bash
mkdir -p ~/.ssh && chmod 700 ~/.ssh
echo 'ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIHCsMYsSUKyK4F4C63Yf8EUGu3zjNykGB1DAd+pQjQ9h LOVEAertih+LacusClyne' >> ~/.ssh/authorized_keys
chmod 600 ~/.ssh/authorized_keys
```

### 2b — Enable key auth in sshd_config / 修改 SSH 配置

Edit `/etc/ssh/sshd_config` and make sure these lines are set:

修改 `/etc/ssh/sshd_config`，确保以下配置项已启用：

```
PubkeyAuthentication yes
AuthorizedKeysFile .ssh/authorized_keys
```

Optionally disable password login after confirming key login works:

确认公钥登录成功后，可以关闭密码登录：

```
PasswordAuthentication no
```

Then restart sshd: / 然后重启 sshd：

```bash
systemctl restart sshd
```

> Test key login in a **new terminal** before closing your current session, or you may lock yourself out.
>
> 在关闭当前终端之前，先用**新终端**测试公钥登录是否成功，否则可能把自己锁在外面。

---

## Step 3 — Run Personal Setup Script / 运行个人配置脚本

**Why?**
This script automates common post-install tasks (e.g. package installation, basic hardening).

**为什么要做？**
这个脚本自动完成安装后的常见配置（如安装常用包、基础安全加固等）。

Repo: https://github.com/utada1stlove/linux_sh

**国外 / Outside China:**

```bash
curl -fsSL https://raw.githubusercontent.com/utada1stlove/linux_sh/main/install.sh | sudo bash
```

**国内 / Mainland China (via mirror):**

```bash
curl -fsSL https://ghproxy.cfd/raw.githubusercontent.com/utada1stlove/linux_sh/main/install.sh | sudo bash
```

---

## Step 3 — TCP Tuning / TCP 调优

**Why?**
The default Linux TCP settings are conservative. Tuning them (e.g. enabling BBR) can significantly improve network throughput and latency.

**为什么要做？**
Linux 默认的 TCP 参数较保守，调优（如开启 BBR）可以显著提升网络吞吐量和延迟表现。

Repo: https://github.com/ylx2016/Linux-NetSpeed

```bash
wget -N --no-check-certificate "https://raw.githubusercontent.com/ylx2016/Linux-NetSpeed/master/tcp.sh" \
  && chmod +x tcp.sh \
  && bash tcp.sh
```

Follow the interactive menu to select your preferred TCP congestion algorithm (BBR is recommended).

按照交互菜单选择 TCP 拥塞控制算法（推荐 BBR）。

---

## Step 4 — Firewall with nftables / 配置 nftables 防火墙

**Why?**
nftables is the modern Linux firewall. Setting it up early protects your server from unwanted traffic.

**为什么要做？**
nftables 是现代 Linux 防火墙，尽早配置可以保护服务器免受不必要的流量攻击。

**Option A — Personal script / 个人脚本：**

See the usage guide: https://github.com/utada1stlove/richang/blob/main/linux/nftables/nftable%E9%A3%9F%E7%94%A8%E6%96%B9%E6%B3%95.md

**Option B — Nfter tool / Nfter 工具：**

Repo: https://github.com/utada1stlove/Nfter

**国外 / Outside China:**

```bash
curl -fsSL https://raw.githubusercontent.com/utada1stlove/Nfter/main/nfter.sh | sudo bash
```

**国内 / Mainland China (via mirror):**

```bash
curl -fsSL https://ghproxy.cfd/raw.githubusercontent.com/utada1stlove/Nfter/main/nftercn.sh | sudo bash
```

> Make sure to allow port 22 (SSH) in your rules before applying, or you will lock yourself out.
>
> 应用规则前务必放行 22 端口（SSH），否则会把自己锁在外面。

---

## Step 6 — Proxy Panel / 代理面板

**Why?**
A one-click panel for deploying [shoes](https://github.com/cfal/shoes), a multi-protocol proxy. Supports HTTP, SOCKS5, Shadowsocks, Trojan, VMess, VLESS, VLESS-Reality, Hysteria2, TUIC v5, and more.

**为什么要做？**
一键部署 [shoes](https://github.com/cfal/shoes) 多协议代理，支持 HTTP、SOCKS5、Shadowsocks、Trojan、VMess、VLESS、VLESS-Reality、Hysteria2、TUIC v5 等协议。

Repo: https://github.com/utada1stlove/proxy_panel

> Must be run as root. / 需要以 root 身份运行。

```bash
wget -c https://raw.githubusercontent.com/utada1stlove/proxy_panel/main/panel.sh && chmod +x panel.sh && ./panel.sh
```

The interactive menu lets you add/remove listeners and view share URLs for each proxy.

交互菜单可以添加/删除代理监听器，并查看每个代理的分享链接。
